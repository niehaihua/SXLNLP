...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        # 使用RNN层，输入维度vector_dim，输出维度vector_dim，隐藏层维度vector_dim
    ...
    def forward(self, x, y=None):
        ...
        # RNN层
        # x = self.rnn(x)[0]  # (batch_size, vector_dim, sen_len)
        # x = x[:, -1, :]  # 取最后一个时间步的输出
        y_pred = self.classify(x)  # (batch_size, vector_dim)不变
        if y is not None:
...
def main():
    ...
    plt.show()
    # 保存模型
    torch.save(model.state_dict(), "../../第三周 深度学习处理文本/week3 深度学习处理文本/model.pth")
    # 保存词表
    writer = open("../../第三周 深度学习处理文本/week3 深度学习处理文本/vocab.json", "w", encoding="utf8")
...
if __name__ == "__main__":
    ...
    predict("../../第三周 深度学习处理文本/week3 深度学习处理文本/model.pth", "vocab.json", test_strings)